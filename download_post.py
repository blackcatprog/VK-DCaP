...
def download_post(id_, count, _photo=0, _music=0, _document=0, _folder=0, _log=0):
	...
	#проверяем создана ли папка, если создана, то скачиваем всё внеё, если не создана, то создаём
	try:
		if FOLDER != 0:
			os.mkdir(FOLDER)
			print(succes + f"Создание папки {FOLDER}")
	...
	if LOG != 0:
		print(succes + "История постов получена успешно")
	...
	if LOG != 0:
		print(succes + "Информация о пользователе/группе собрана успешно")
...
<html>
	...
	<body style='background-color: #333'>
		<div style='width: 750px; margin: 10px auto 10px auto; background-color: #5381B9;border-radius: 10px'>
			<center><a href='{link_user}' target='_blank'><img src='ava.jpg' style='border-radius: 100px; margin-top: 20px'></center></a>
	...
	try:
			...
			if "attachments" in j.keys():
				if j["attachments"][0]["type"] == "photo":
					if DOWNLOAD_PHOTO == 0:
						...
						html2 += f'''<div style='margin-bottom: 10px; width: 750px; box-sizing: content-box; background-color: #5281B9; border-radius: 10px; font-family: sans-serif; margin-left: auto; margin-right: auto;'>
										<span style='display: inline-block; padding: 10px'>{j["text"]}<br></span>
										<img src='{url_photo}' style='height: 100px; padding: 10px; border-radius: 20px'>
						...
						if FOLDER == 0:
							...
							with open(f"{FOLDER}/{name_photo}", "wb") as file:
								file.write(url_photo.content)
						html2 += f'''<div style='margin-bottom: 10px; width: 750px; box-sizing: content-box; background-color: #5281B9; border-radius: 10px; font-family: sans-serif; margin-left: auto; margin-right: auto;'>
										<img src='{name_photo}' style='height: 100px; padding: 10px; border-radius: 20px'>
		...
		if LOG != 0:
			print(succes + "Пост(ы) отформатирован(ы) успешно")
	except KeyboardInterrupt:
		print(error + "Выход")
		sys.exit(1)
	...
	if FOLDER == 0:
		...
		with open(f"{FOLDER}/posts.html", "w", encoding="utf-8") as file:
			file.write(html_join)
			if LOG != 0:
				print(succes + "Пост(ы) скачан(ы)")